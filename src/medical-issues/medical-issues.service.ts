...
import { MedicalIssue, MedicalIssueDocument } from './schema/medical-issue.schema';
...
export class MedicalIssuesService {
  constructor(
    @InjectModel(MedicalIssue.name) private issueModel: Model<MedicalIssueDocument>,
  ) {}

  async create(createMedicalIssueDto: CreateMedicalIssueDto): Promise<MedicalIssue> {
    const { name } = createMedicalIssueDto;

    const existing = await this.issueModel.findOne({ name });
    if (existing) {
      throw new BadRequestException(`A medical issue with the name: ${name} already exists`);
    }
  ...
  async update(id: string, updateDto: UpdateMedicalIssueDto): Promise<MedicalIssue> {
    const issue = await this.issueModel.findById(id);
    if (!issue) {
      throw new NotFoundException('Medical issue not found');
    }
    ...
    if (updateDto.name && updateDto.name !== issue.name) {
      ...
      if (existing) {
        throw new BadRequestException(`A medical issue with the name:${updateDto.name} already exists`);
      }
    }
