...
} from './schema/loanRepayment.schema';
...
export class LoanService {
  ...
  async repayLoan(
    ...
    if (remainingBalance === 0) {
      ...
      await this.walletService.updateLoanEligibility(
        userId,
        defaultEligibility,
      );
  ...
  async getLoanHistory(userId: string) {
    ...
    return {
      ...
      history,
      loanBalance: {
        remainingBalance: loanBalances.reduce(
          (acc, loan) => acc + loan.remainingBalance,
          0,
        ), // Total remaining balance of all loans
      },
