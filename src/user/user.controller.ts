...
export class UserController {
  ...
  @ApiResponse({
    status: 200,
    description: 'List of nearby specialized users',
    type: [User],
  })
    ...
    schema: {
      ...
      properties: {
        ...
        specialty: {
          ...
          nullable: true,
          description:
            'Specialization of the user (required for medical practitioners)',
        },
      },
  ...
  async loginUser(@Body() loginDto: LoginDto & { role: string }) {
    ...
    if (!user) {
      throw new UnauthorizedException(
        'No account found for this user, please signup',
      );
    }

    if (user.role !== role) {
      throw new UnauthorizedException(
        `Invalid email or password for selected role`,
      );
    }
  ...
  async requestReset(
    @Body() body: { email: string; role: 'client' | 'medical_practitioner' },
  ) {
